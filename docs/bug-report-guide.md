...
**唯一的反馈渠道是 [GitHub Issues](https://github.com/andyhang1980/Xchat/issues)。**
...
1. **你的问题是否已经有人提过？** 搜索 [已有 Issues](https://github.com/andyhang1980/Xchat/issues) 和 [常见问题](faq.md) 避免重复提交。
2. **你的模块版本是否最新？** 请尝试升级到最新的 [CI 构建](installation.md#下载) 再确认问题是否仍然存在。
3. **是否排除了其他模块干扰？** 在 LSPosed 中暂时仅勾选 Xchat 并重启微信重试, 排除其他 Xposed 模块的冲突。
...
```none
/sdcard/Android/data/<宿主包名>/Xchat/logs/
```
