...
object WeLogger {
    ...
    private fun getOrRotateWriter(): FileWriter? {
        ...
        val logPath = logsDir / "xchat-${dateFmt.format(today)}.log"
    ...
    private fun deleteOldLogs(logsDir: java.nio.file.Path) {
        runCatching {
            ...
            val logFileRegex = Regex("""xchat-(\d{4}-\d{2}-\d{2})\.log""")
    ...
    /**
     * All run-log files (`xchat-yyyy-MM-dd.log`), newest first. Flushes the active writer first so
    ...
    val allLogFiles: List<java.nio.file.Path>
        get() {
            ...
            val regex = Regex("""xchat-\d{4}-\d{2}-\d{2}\.log""")
            return runCatching {
