...
import io.github.xchat.features.api.core.models.IWeContact
import io.github.xchat.features.api.ui.WeStartActivityApi
import io.github.xchat.features.core.ClickableFeature
import io.github.xchat.features.core.Feature
import io.github.xchat.features.items.contacts.CustomLocalFriendAvatars
import io.github.xchat.ui.content.AlertDialogContent
import io.github.xchat.ui.content.BaseContactSelector
...
import io.github.xchat.utils.serialization.DefaultJson
...
object AggregateChats : ClickableFeature(),
    ...
    const val FOLDER_PREFIX = "xchat_folder_"
    ...
    // Our folder rows (xchat_folder_XXX) pass those guards and reach the share machinery, which
    ...
    // (xchat_folder_XXX) reach that same path with a non-existent username → crash downstream.
    ...
    private fun startRefreshThread() {
        val thread = HandlerThread("xchat-folder-refresh").also {
            it.start()
            refreshThread = it
        }
    ...
    private fun syncFolder(folder: ChatFolder) {
        ...
        WeDatabaseApi.execStatement(
            ...
            VALUES (?, 3, ?, 0, ?)
            """.trimIndent(),
            arrayOf(folder.id, System.currentTimeMillis() / 1000, "http://xchat.local/avatar/${folder.id}")
        )
